...
void Init(GLFWwindow* window) {
    ...
    GLuint fb_tex_id = framebuffer.Init(window_width, window_height);
    // TODO: initialize shinyfloor with the FB texture
    shinyfloor.Init(fb_tex_id);
}
...
void Display() {
    ...
    vec3 cam_mirror(2.0f, 2.0f, -2.0f);
    // TODO: create new VP for mirrored camera
    mat4 mirror_view = lookAt(cam_mirror, cam_look, cam_up);
    mat4 mirror_projo = projection_matrix * mirror_view;
    // TODO: render the cube using the mirrored camera
    framebuffer.Clear();
    framebuffer.Bind();
        cube.Draw(mirror_projo);
    framebuffer.Unbind();
