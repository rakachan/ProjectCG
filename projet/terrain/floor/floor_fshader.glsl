...
void main() {
    ...
    if (height<0) {
        ...
        vec3 normal_mv = normalize(texture(normalmap, 10.0f*uv+vec2((0.07f*(time+0.1f*sin(time)*sin(time)))), -0.1f*time).rgb);
        ...
        float cos_angle = abs(dot(normal_mv, view_dir));
        vec2 distortion = coord*height*0.3f*normal_mv.xz;
        color =  vec4(diffuse, 0) + vec4(specular, 0) + vec4(texture(ref, coord+distortion).rgb, 0);
        color.a = mix(0.8, 0.5, cos_angle);
        //color = vec4(normal_mv,1);
    } else {
        discard;
    }
}
